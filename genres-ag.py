...
model_run_name = 'baseline_ag_w_pretrain_200_cl_fib_1-10'
BEAM_SIZE = 5
...
def add_eos_to_examples(example):
    example['input_text'] = 'generate assert: %s </s>' % example['buggy']#.lower()
...
#valid_dataset = nlp.load_dataset('ag_dataset_script.py', split=nlp.Split.TEST)
df = pd.read_csv("/home/amharris/aug/ag_test.tsv",header=None,sep='\t')
df = df.rename(columns={0:'buggy',1:'fixed'})

### convert to Huggingface dataset
valid_dataset = Dataset(pa.Table.from_pandas(df))
